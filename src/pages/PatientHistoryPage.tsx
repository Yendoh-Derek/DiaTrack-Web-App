...
const PatientHistoryPage = () => {
  ...
  const predictions: PredictionResult[] = patientId ? getPredictionsForPatient(patientId) : [];
    ...
    return (
      <AppLayout>
        <EmptyState
          ...
          action={{ label: 'Back to Patients', onClick: () => navigate('/patients') }}
        />
      </AppLayout>
    );
  }
  ...
  const exportCsv = () => {
    const rows = [['Date', 'Risk %', 'Level', 'Result', 'Recommendations'],
      ...predictions.map(p => [
    ...
    >
      <header className="mb-6">
        ...
        <div className="flex flex-wrap items-baseline gap-x-4 gap-y-1">
          ...
          <span style={{ fontSize: 'var(--text-body-md)', color: 'var(--color-text-secondary)' }}>
            DOB: {patient.date_of_birth ?? '—'}
          </span>
          <span className="tabular-nums" style={{ fontSize: 'var(--text-body-md)', color: 'var(--color-text-secondary)' }}>
            MRN: {patient.patient_id}
          </span>
        </div>
      </header>
      ...
      {latest ? (
        <div className="dashboard-grid mb-6">
          ...
          <div className="clinical-card col-8">
            ...
            <ModelInterpretability contributions={latest.feature_contributions} />
          </div>
        </div>
      ) : (
        <EmptyState
          ...
          action={{ label: 'Run Assessment', onClick: () => navigate(`/assessment?patientId=${patient.id}`) }}
        />
      )}
        ...
        <>
              ...
              {input ? (
                <dl className="grid grid-cols-2 gap-x-4 gap-y-3">
                  {[
                    ...
                    { label: 'Gender', value: input.gender === 1 ? 'Male' : 'Female' },
                    { label: 'BMI', value: `${input.bmi.toFixed(1)} kg/m²` },
                    { label: 'HbA1c', value: `${input.HbA1c_level}%` },
                    { label: 'Blood Glucose', value: `${input.blood_glucose_level} mg/dL` },
                    { label: 'Hypertension', value: input.hypertension ? 'Yes' : 'No' },
                    { label: 'Heart Disease', value: input.heart_disease ? 'Yes' : 'No' },
                    { label: 'Smoking', value: input.smoking_history },
                  ].map(item => (
                    <div key={item.label}>
                      <dt className="field-label">{item.label}</dt>
                      <dd className="tabular-nums mt-1" style={{ fontSize: 'var(--text-body-md)' }}>
                        {item.value}
                      </dd>
                    </div>
                  ))}
          ...
          <div className="dashboard-grid">
            <div className="clinical-card col-6">
              ...
              <p className="mt-3" style={{ fontSize: 'var(--text-body-sm)', color: 'var(--color-text-tertiary)' }}>
