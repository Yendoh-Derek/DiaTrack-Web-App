...
import { getPredictions, getPatients, getPatientById } from '@/stores/demoStore';
...
const HistoryPage = () => {
  ...
  const chartData = useMemo(() =>
    [...predictions].reverse().slice(-20).map(p => ({
  ...
  return (
    <AppLayout>
      <header className="mb-6">
        <h1 className="font-semibold" style={{ fontSize: 'var(--text-display)' }}>All Assessments</h1>
      ...
      {predictions.length === 0 ? (
        <EmptyState
          ...
          action={{ label: 'Run First Assessment', onClick: () => navigate('/assessment') }}
        />
      ) : (
        <>
          {chartData.length > 1 && (
                ...
                {predictions.map(p => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td style={{ color: 'var(--color-text-secondary)' }}>{p.prediction_result}</td>
                      <td>
                        {patient && (
                          <Button
                            size="sm"
